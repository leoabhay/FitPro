  ...
  return new Promise<void>((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!database.objectStoreNames.contains("diet")) {
        const dietStore = database.createObjectStore("diet", {
          keyPath: "id",
          autoIncrement: true,
        });
      ...
      if (!database.objectStoreNames.contains("goals")) {
        const goalStore = database.createObjectStore("goals", {
          keyPath: "id",
          autoIncrement: true,
        });
      ...
      if (!database.objectStoreNames.contains("water")) {
        const waterStore = database.createObjectStore("water", {
          keyPath: "id",
          autoIncrement: true,
        });
...
export async function saveDietEntry(entry: DietEntry) {
  return new Promise((resolve, reject) => {
...
export async function getDietEntries(date?: string) {
  return new Promise<DietEntry[]>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error);
    request.onsuccess = () => resolve(request.result as DietEntry[]);
  });
}
...
export async function updateGoal(id: number, goal: Goal) {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction(["goals"], "readwrite");
    const store = transaction.objectStore("goals");
...
export async function getWaterLogs(date?: string) {
  return new Promise<WaterLog[]>((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error);
    request.onsuccess = () => resolve(request.result as WaterLog[]);
  });
}
